    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted text-sm">Нийтлэл болон сайтад ашиглагдах зураг, материалын нэгдсэн удирдлага</p>
        </div>
        <button 
        ...
        >
          ...
          <form onSubmit={handleAddMedia} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="space-y-1.5 md:col-span-1">
              <label className="text-text-main text-xs font-semibold">Зургийн нэр</label>
              <input 
                type="text"
                required
                value={nameInput}
            ...
            <div className="space-y-1.5 md:col-span-1">
              <label className="text-text-main text-xs font-semibold">Зургийн холбоос (Image URL)</label>
              <input 
                type="text"
                required
                value={urlInput}
      ...
      <div className="bg-surfaceHighlight border border-border rounded-2xl p-4 flex flex-col sm:flex-row gap-4 items-center justify-between">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-background border border-border rounded-xl text-text-main text-xs outline-none focus:border-brand-purple/50 transition-colors"
          />
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-background border border-border rounded-xl p-1">
          <button 
            ...
            className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-brand-purple text-white' : 'text-text-muted hover:text-text-main'}`}
          ...
          <button 
            ...
            className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-brand-purple text-white' : 'text-text-muted hover:text-text-main'}`}
      ...
      {filteredMedia.length === 0 ? (
        <div className="bg-surface border border-border rounded-2xl p-12 text-center space-y-2">
          <ImageIcon size={44} className="text-slate-600 mx-auto" />
          <h3 className="text-text-main font-bold text-md">Медиа файл олдсонгүй</h3>
          <p className="text-text-muted text-xs">Хайлтын утгад тохирох зураг олдсонгүй.</p>
        </div>
      ) : viewMode === 'grid' ? (
        /* Grid Mode */
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {filteredMedia.map((item) => (
            <div 
              key={item.id} 
              className="bg-[#131B2E]/30 border border-border rounded-2xl overflow-hidden group hover:border-brand-purple/20 transition-all flex flex-col h-full"
            >
              {/* Image box */}
              <div className="aspect-video bg-slate-900/80 relative overflow-hidden flex-shrink-0">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button 
                    ...
                    className="p-2 bg-white/10 hover:bg-white/20 text-text-main rounded-lg transition-colors"
              ...
              <div className="p-3 flex-1 flex flex-col justify-between">
                <p className="text-xs font-bold text-text-main truncate" title={item.name}>{item.name}</p>
                  ...
                  <tr key={item.id} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="p-4 pl-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg overflow-hidden bg-slate-900 border border-border flex-shrink-0">
                          <img src={item.url} alt="" className="w-full h-full object-cover" />
                        </div>
                        <span className="text-xs font-bold text-text-main truncate max-w-[150px]">{item.name}</span>
                      </div>
                    </td>
                    <td className="p-4">
                      <span className="text-xs font-mono text-text-muted select-all truncate max-w-[250px] block">
                        {item.url}
                      </span>
                    </td>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-1.5 opacity-80 group-hover:opacity-100">
                        <button 
                          onClick={() => handleCopy(item.id, item.url)}
                          className="p-1.5 hover:bg-brand-purple/10 text-text-muted hover:text-brand-purple rounded-lg transition-all"
                          title="Хуулах"
                        >
                          {copiedId === item.id ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                        </button>
                        <button 
                          onClick={() => handleDelete(item.id)}
                          className="p-1.5 hover:bg-red-500/10 text-text-muted hover:text-red-400 rounded-lg transition-all"
                          title="Устгах"
                        >
                          <Trash2 size={14} />
                        </button>
