  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-text-muted text-sm">Нийтлэлүүдэд хавсаргаж, хайлтын оновчлолыг сайжруулах түлхүүр үгсийг удирдах хэсэг</p>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-surface border border-border rounded-2xl p-5 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-border">
              <h3 className="text-text-main font-bold text-sm">Идэвхтэй шошгонууд ({tags.length})</h3>
              <button 
                onClick={fetchTags}
            ...
            {loading ? (
              <div className="py-12 text-center">
                ...
                <p className="text-text-muted text-xs mt-3">Шошгонуудыг уншиж байна...</p>
              </div>
            ) : tags.length === 0 ? (
              <div className="py-12 text-center space-y-2">
                <Hash size={32} className="text-slate-600 mx-auto" />
                <p className="text-text-muted text-sm">Одоогоор ямар нэг шошго үүсгэгдээгүй байна.</p>
              </div>
            ) : (
              <div className="flex flex-wrap gap-2.5">
                {tags.map((tag) => (
                  <div 
                    key={tag.id}
                    className="pl-3 pr-2.5 py-2 bg-surfaceHighlight border border-border rounded-xl flex items-center gap-2 group hover:border-brand-purple/20 transition-all"
                  >
                    <span className="text-text-main text-xs font-semibold flex items-center gap-1">
                      <span className="text-brand-purple">#</span>
                      {tag.name}
                    </span>
                    <span className="bg-surfaceHighlight text-[10px] px-1.5 py-0.5 rounded-md text-text-muted font-bold group-hover:text-text-main">
                      {tag.articleCount}
                    </span>
                    ...
                    <div className="flex items-center gap-0.5 border-l border-border pl-1">
                      <button 
                        ...
                        className="p-1 text-text-muted hover:text-brand-purple rounded transition-colors"
                        title="Засах"
                      >
                        <Edit2 size={11} />
                      </button>
        ...
        <div className="bg-surface border border-border rounded-2xl p-5 h-fit space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-text-main text-xs font-semibold">Шошгоны нэр</label>
              <div className="relative">
                ...
                <input 
                  ...
                  required
                  placeholder="AI"
                  className="w-full pl-7 pr-4 py-2.5 bg-background border border-border rounded-xl text-text-main text-sm outline-none focus:border-brand-purple/50 transition-colors"
                />
              </div>
            </div>
